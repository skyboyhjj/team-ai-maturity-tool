...
```
team-ai-maturity-tool/
├── .github/workflows/      # GitHub Actions 部署配置
│   └── deploy.yml          # 自动部署到 GitHub Pages
├── docs/                   # 文档目录
│   ├── 产品规划书.md                 # 产品规划书 v1.1
│   ├── AI能力提升行动清单与安全FAQ.md  # AI能力提升行动清单与安全FAQ
│   ├── AI采用五阶段_旋量太极读解.md   # AI采用五阶段理论解析
│   ├── Boris_Cherny_AI采用步骤.md     # Boris Cherny AI采用步骤原文
│   ├── Boris_Cherny_AI采用五阶段框架.md # AI采用五阶段框架说明
│   ├── 使用指南.md                    # 工具使用指南
│   ├── 自评问卷_增强版v2.md           # 自评问卷增强版
│   └── 问题修正建议.md                # 测试问题修正建议
├── scripts/                # Python脚本目录（阶段2自动采集）
│   ├── auto_collect.py     # 每周自动采集8个客观指标
│   ├── auto_pipeline.py    # 合并自动采集和人工评分
│   ├── feishu_archive.py   # 飞书多维表格归档
│   └── requirements.txt    # Python依赖说明
├── tests/                  # 测试目录
│   └── test_calculations.py # 评分计算逻辑测试
├── .gitignore              # Git忽略配置
├── LICENSE                 # MIT许可证
├── README.md               # 项目说明文档
├── index.html              # 主应用页面（纯静态HTML）
└── screenshot.png          # 项目截图
```
...
**新增文档（docs/）：**
- 产品规划书 v1.1 - 包含项目里程碑完成情况、需求变更记录、资源分配调整、风险评估更新及下一阶段任务规划
- AI能力提升行动清单与安全FAQ - 5个阶段的可落地改进建议及8个高频合规问题解答
- AI采用五阶段_旋量太极读解 - 基于旋量-太极模型的AI采用五阶段理论解析
- 使用指南 - 工具详细使用说明
- 自评问卷_增强版v2 - 含赋分标准和旋量-太极诊断的完整问卷
- 问题修正建议 - 测试过程中发现的问题及修正方案

**新增脚本（scripts/）：**
- `auto_collect.py` - 每周自动采集8个客观指标（Git统计、飞书API、CI/CD检测、Agent统计）
- `auto_pipeline.py` - 合并自动采集指标和人工评分，生成完整成熟度报告
- `feishu_archive.py` - 将成熟度报告归档到飞书多维表格
- `requirements.txt` - Python脚本依赖说明

**功能优化：**
- 诊断报告显示期间隐藏所有浮动按钮，避免遮挡报告内容
- 反馈弹窗显示期间隐藏所有浮动按钮
- 滚动事件监听器中添加业务状态检查，防止按钮在弹窗期间重新显示
...
| 分数区间 | 阶段名称 | 核心特征 |
|----------|----------|----------|
| 15 – 24 | 🛠️ **辅助模式** | 零星使用 AI 工具，无系统化流程，AI 作为"外挂"存在 |
